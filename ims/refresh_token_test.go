...
func TestRefreshToken(t *testing.T) {
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if v := r.PostForm.Get("grant_type"); v != "refresh_token" {
			t.Fatalf("invalid grant type: %v", v)
		}
		if v := r.PostForm.Get("refresh_token"); v != "refreshToken" {
...
func TestRefreshTokenError(t *testing.T) {
	...
	_, err = c.RefreshToken(&ims.RefreshTokenRequest{
		RefreshToken: "refreshToken",
		ClientID:     "clientID",
		ClientSecret: "clientSecret",
	})
...
func TestRefreshTokenNoRefreshToken(t *testing.T) {
	...
	if _, err := c.RefreshToken(&ims.RefreshTokenRequest{
		ClientID:     "clientID",
		ClientSecret: "clientSecret",
	}); err == nil {
		t.Fatalf("no error returned")
	} else if err.Error() != "missing refresh token" {
		t.Fatalf("invalid error: %v", err)
	}
}

func TestRefreshTokenNoClientID(t *testing.T) {
	c, err := ims.NewClient(&ims.ClientConfig{
		URL: "http://ims.endpoint",
	})
	if err != nil {
...
func TestRefreshTokenNoClientSecret(t *testing.T) {
	c, err := ims.NewClient(&ims.ClientConfig{
		URL: "http://ims.endpoint",
	})
	if err != nil {
